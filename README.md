...
- **S:** Один класс — одна зона ответственности (`UnitStatsCalculator`, `UnitSpawnService`, системы ECS и т.д.).
- **O:** Новые цвета/формы/размеры — через enum и SO, без изменения формул расчёта.
- **L/D:** Зависимости от интерфейсов (`IMainMenuView`, `IGameUIView`, `IUnitViewFactory`, `IBattleEventBus`, `ISceneLoader`).
- В коде нет `FindObjectOfType` и прямого `GetComponent` для связки; ссылки на View и настройки — через DI или SO.
